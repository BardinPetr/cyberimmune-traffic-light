...
trafficlight::IExternalControl::IExternalControl(NkKosTransport *transport, nk_iid_t riid) : transport(transport),
                                                                                             riid(riid) {
}

trafficlight::IExternalControl::IExternalControl(NkKosTransport *transport, const char *endpoint_name) : transport(
        transport) {
...
nk_err_t trafficlight::IExternalControl::GetStatus(nk_uint8_t &out_maintenance, nk_uint8_t &out_manual) {
    ...
    out_maintenance = res.maintenance;

    out_manual = res.manual;


    return rcOk;
}

nk_err_t trafficlight::IExternalControl::GetMode(nk_uint8_t in_id, nk_uint32_t &out_mode) {
...
nk_err_t
trafficlight::IExternalControl::GetErrors(nk_uint8_t in_id, std::string &out_severity, DirectionError &out_errors) {
    ...
    out_errors = res.errors;


    // begin arg res.severity IDLTypeString

    rtl_size_t cnt_out_severity_0;
    nk_char_t *ptr_out_severity_0 = nk_arena_get(
            nk_char_t,
            &resArena,
            &res.severity,
            &cnt_out_severity_0
    );
    out_severity.assign(ptr_out_severity_0);

    // end arg res.severity
